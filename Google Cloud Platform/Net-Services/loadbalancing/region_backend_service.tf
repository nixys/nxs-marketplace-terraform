resource "google_compute_region_backend_service" "region_backend_service" {
  for_each = var.region_backend_services
  ...
  health_checks                   = try([for health_checks in each.value.health_checks : google_compute_region_health_check.region_health_check[health_checks].id], [])
  ...
  depends_on = [google_compute_region_health_check.region_health_check]
}
