resource "google_compute_url_map" "url_map" {
  ...
  default_service             = try(google_compute_backend_service.backend_service[each.value.default_service].id, null)
  ...
  dynamic "path_matcher" {
    ...
    content {
      ...
      default_service = try(google_compute_backend_service.backend_service[path_matcher.value.default_service].id, null)

      dynamic "path_rule" {
        for_each = try(path_matcher.value.path_rule, [])
        content {
          paths   = path_rule.value.paths
          service = try(google_compute_backend_service.backend_service[path_rule.value.service].id, null)
        }
      }
    }
  }
